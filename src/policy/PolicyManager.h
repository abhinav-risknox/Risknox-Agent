          ...
          else if (policyType == "patch")
{
    ...
    // Capture commandId as well
    std::thread([wm, policyData, cb, commandId]()
    {
        // Spawn the patch worker
        wm->spawnWorker("rp-patch.exe", "rp-patch", false);

        auto patches = std::make_shared<std::vector<nlohmann::json>>();

        wm->streamEvents("rp-patch", policyData,
            [cb, patches, commandId](const nlohmann::json &event)
            {
                std::string type = event.value("type", "");
                LOG_INFO("PolicyManager [rp-patch]: {}", event.dump());

                // Collect patch update events
                if (type == "update" && event.contains("data"))
                {
                    patches->push_back(event["data"]);
                }

                // On completion, build full report
                if (cb && type == "complete")
                {
                    nlohmann::json report = event;
                    report["command_id"] = commandId;
                    report["patches"] = *patches;
                    report["total"] = patches->size();

                    LOG_INFO("FINAL PATCH REPORT = {}", report.dump(2));

                    // Persist full JSON, not just summary
                    cb("patch_scan", report);
    ...
    result = true;
            ...
            else if (policyType == "antivirus")
            {
                ...
                else
                {
                    ...
                    // and are picked up when the current worker finishes.
                    {
                        ...
                        if (!path.empty())
                            avPendingPaths_.push_back(path);
         ...
         */
        void
        sendStatusReport(nlohmann::json baseData = nlohmann::json::object())
        {
    ...
    private:
        ...
        void runAvScanLoop(WorkerManager *wm, StatusReportCallback cb)
        {
            while (true)
            {
                ...
                if (paths.empty())
                    continue; // all vanished, check for more
