...
namespace ResolutePulse
{
                                       ...
                                       const std::string &payload)
    {
        ...
        if (lastAgentId_.empty())
            lastAgentId_ = agentId;
    ...
    void AgentHandler::handleStatusReport(SSL *ssl, const std::string &agentId,
                                          const std::string &payload)
    {
        LOG_INFO("################ NEW HANDLESTATUSREPORT ################");
        ...
        try
        {
            ...
            LOG_INFO("RAW PAYLOAD = {}", payload);
            LOG_INFO("REPORT TYPE = {}", report.reportType);
            LOG_INFO("REPORT DATA = {}", report.reportData);

            if (report.reportType == "patch_scan")
            {
                try
                {
                    auto patchJson = nlohmann::json::parse(report.reportData);

                    server_->setLatestPatchReport(agentId, patchJson);

                    LOG_INFO("Live patch report cached for {}", agentId);
                }
                catch (const std::exception &ex)
                {
                    LOG_ERROR("Failed to parse patch report: {}", ex.what());
                }
            }

            // Store status report in DB for dashboard queries
            db_.storeStatusReport(agentId, report.reportType, report.reportData);
            LOG_INFO("===== storeStatusReport SUCCESS =====");
        }
        catch (const std::exception &ex)
        {
            LOG_ERROR("StatusReport exception: {}", ex.what());
        }
